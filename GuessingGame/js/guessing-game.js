...
guessButton.addEventListener('click', () => {
    ...
    if(input.value != dratiniNr) {
        alert("Bummer.. You guessed " + input.value + " and the secret number was " + dratiniNr + ".")
    }
    
    // Awesome! You number (actual number) was correct. You can be named many things, hungry not being one of them.
    if(input.value == dratiniNr) {
        alert("Awesome! You number " + input.value + " was correct. You can be named many things, hungry not being one of them.");
    }
});